...
.brand {
  font-size: 25px; font-weight: 800; color: var(--gold);
}
...
.action:hover:not(:disabled) {
  transform: translateY(-1px); border-color: var(--gold); background: #18394e;
}
...
.tags span, .pill {
  display: inline-block; border: 1px solid #426078; border-radius: 20px; padding: 1px 7px; margin: 2px;
  color: var(--blue); font-size: 11px;
}
.regions { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 8px; }
.region {
  border: 1px solid #294b63; border-radius: 8px; padding: 9px; background: #0e2637;
}
...
.goal {
  display: flex; justify-content: space-between; border-bottom: 1px solid #1e394d; padding: 7px 0;
}
.done { color: var(--green); }
.modal { display: none; position: fixed; inset: 0; background: #02080dcc; place-items: center; padding: 18px; z-index: 5; }
.modal.open { display: grid; }
.modal-box {
  max-width: 620px; width: 100%; background: #112b3d; border: 1px solid var(--gold); border-radius: 12px;
  padding: 22px; box-shadow: 0 20px 70px #000b;
}
.choices { display: grid; gap: 8px; }
.choice { text-align: left; padding: 12px; }
.choice b { display: block; color: var(--gold); }
.toast {
  position: fixed; right: 20px; bottom: 20px; background: #15384c; border: 1px solid var(--green);
  padding: 10px 14px; border-radius: 8px; display: none; z-index: 9;
}
